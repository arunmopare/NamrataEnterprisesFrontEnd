...
export class ContactUsComponent implements OnInit {
  ...
  myflag = false;
  durationInSeconds = 5;
  url = `https://salty-dusk-84006.herokuapp.com/send`
  emailPattern = '^[a-z0-9._%+-]+@[a-z0-9.-]+.[a-z]{2,4}$';


  constructor(private http:HttpClient , private formBuilder: FormBuilder,private _router: Router,private _snackBar: MatSnackBar) { 
  ...
  ngOnInit() {
    this.registerDesktopForm = this.formBuilder.group({
      ...
      lastname: ['', [Validators.required,Validators.pattern('^[a-zA-Z]+( [a-zA-Z_]+)*$')]],
  ...
  signupDesk(){
    ...
    console.log("Success")
    this.http.post(this.url,this.registerDesktopForm.value).subscribe((responce:any)=>{
  ...
  signupMobile(){
    ...
    this.http.post(this.url,this.registerMobileForm.value).subscribe((responce:any)=>{
      this.openSuccessSnackBar();
...
@Component({
  ...
  template: `<span class="example-pizza-party text-center">
        <h2><span style="color:white">Response Sent<i style="margin-left: 10px; color:green" class="fa fa-check da-2x" aria-hidden="true"></i> </span></h2>
</span>`,
  styles: [`
    .example-pizza-party {
      color: green;
...
@Component({
  selector: 'snack-bar-component-example-snack',
  template: `<span class="example-pizza-party text-center">
        <h2><span style="color:white">Something Went Wrong.<i style="margin-left: 10px; color:red;" class="fa fa-times fa-2x" aria-hidden="true"></i> </span></h2>
</span>`,
  styles: [`
    .example-pizza-party {
      color: red;
